    ...
    public String officerQueryPost(@RequestParam String unitId, Model model){
        ...
        if (unitId.length()>0){
            ...
            if (unitRepo.existsById((long) id)){
                persons = personRepo.findAllByRank_isOfficerAndServeUnitId(false, id);
            }
        }
        else {
            persons = personRepo.findAllByRank_isOfficer(false);
        }
